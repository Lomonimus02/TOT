...
// Match the old social-links block (with generic SVGs or spans)
const oldBlockRegex = /<div class="social-links">[\s\S]*?<\/div>/g;

const scripts = [
    path.join(__dirname, 'add-footer-to-all-pages.js'),
    path.join(__dirname, 'add-footer-to-pages.js'),
    path.join(__dirname, 'update-footer-in-pages.js'),
];

scripts.forEach(file => {
    let content = fs.readFileSync(file, 'utf-8');
    // Only replace social-links blocks that are inside the footerHTML template strings
    const matches = content.match(oldBlockRegex);
    if (matches && matches.length > 0) {
        // Replace the first occurrence (inside the footer template)
        content = content.replace(oldBlockRegex, (match, offset) => {
            // Only replace if not already the new block
            if (match.includes('viewBox="0 0 1024 1024"')) return match;
            return newSocialBlock;
        });
