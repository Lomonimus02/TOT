...
class EditMode {
    ...
    setupEditButton() {
        ...
        document.addEventListener('click', (e) => {
            if (e.target && (e.target.id === 'floatingEditBtn' || e.target.closest('#floatingEditBtn'))) {
                e.preventDefault();
                this.toggleEditMode();
            }
        });
    ...
    makeElementsEditable() {
        ...
        additionalSelectors.forEach(selector => {
            ...
            elements.forEach((element, index) => {
                // Пропускаем элементы в навигации и модальных окнах
                if (element.closest('.navbar') || element.closest('.modal')) {
                    return;
                }
    ...
    showEditToolbar() {
        ...
        toolbar.innerHTML = `
            <div class="toolbar-content">
                ...
                <div class="toolbar-section">
                    <button class="toolbar-btn" id="addTextBlockBtn" title="Добавить новый текстовый блок">
                        ➕ Текст
                    </button>
                    <button class="toolbar-btn" id="addImageBtn" title="Добавить изображение">
                        🖼️ Изображение
                    </button>
                </div>
                <div class="toolbar-section">
        ...
        document.getElementById('addTextBlockBtn').addEventListener('click', () => this.addTextBlock());
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.data.changes) {
                    // Контент уже применен на сервере через middleware
                    console.log('Существующий контент загружен');
                }
            }
        } catch (error) {
    ...
    addTextBlock() {
        ...
        const content = prompt('Введите текст для нового блока:');
        if (!content) return;

        const newBlock = document.createElement('div');
        newBlock.className = 'editable-text-block';
        newBlock.innerHTML = content;

        // Добавляем в конец основного контента
        const mainContent = document.querySelector('.page-content .content-section') ||
        ...
        if (mainContent) {
            ...
            const elementId = `text-block-${Date.now()}`;

            // Делаем элемент редактируемым
            this.makeElementEditable(newBlock, elementId);

            // Сразу добавляем в несохраненные изменения
            this.unsavedChanges.set(elementId, {
                original: '',
                current: newBlock.outerHTML,
                element: newBlock
            });

            // Сразу выделяем и начинаем редактирование
            this.selectElement(newBlock);
            setTimeout(() => {
                this.startEditing(newBlock);
            }, 100);

            this.updateToolbarStatus();
            this.showNotification('Текстовый блок добавлен. Кликните для редактирования.', 'success');
        }
    }
    ...
    addImage() {
        ...
        // Создаем модальное окно для выбора способа добавления изображения
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="upload-options">
                        <button class="upload-option" id="uploadFromFile">
                            📁 Загрузить с компьютера
                        </button>
                        <button class="upload-option" id="selectFromGallery">
        ...
        modal.querySelector('#uploadFromFile').addEventListener('click', () => {
            modal.remove();
            this.uploadFromFile();
        });

        modal.querySelector('#selectFromGallery').addEventListener('click', () => {
            this.showGallery(modal);
        });
    }
    ...
    // Показать галерею существующих изображений
    async showGallery(modal) {
        const galleryContainer = modal.querySelector('#galleryContainer');
        const galleryGrid = modal.querySelector('#galleryGrid');

        galleryContainer.style.display = 'block';
        galleryGrid.innerHTML = '<div class="loading">Загрузка изображений...</div>';

        try {
            // Загружаем список изображений
            const response = await fetch(`${this.apiBaseUrl}/api/images/gallery`, {
                headers: {
                    'Authorization': `Bearer ${this.token}`
                }
            });
            ...
            if (response.ok) {
                ...
                this.renderGallery(galleryGrid, result.images, modal);
            } else {
                galleryGrid.innerHTML = '<div class="error">Ошибка загрузки галереи</div>';
            }
        } catch (error) {
    ...
    // Отрисовка галереи
    renderGallery(container, images, modal) {
        ...
        images.forEach(image => {
            ...
            imageItem.addEventListener('click', () => {
                this.insertImageIntoPage(image.url, image.name);
                modal.remove();
            });
    ...
    async uploadImage(file) {
        try {
            ...
            if (response.ok) {
                const result = await response.json();
                this.insertImageIntoPage(result.imageUrl, file.name);
    ...
    insertImageIntoPage(imageUrl, originalName = '') {
        const imageContainer = document.createElement('div');
        imageContainer.className = 'editable-image-container';
        ...
        img.alt = originalName || 'Загруженное изображение';
        ...
        if (mainContent) {
            ...
            // Делаем контейнер редактируемым
            this.makeElementEditable(imageContainer, elementId);

            // Добавляем в несохраненные изменения
            this.unsavedChanges.set(elementId, {
                original: '',
                current: imageContainer.outerHTML,
                element: imageContainer
            });

            // Сразу выделяем новое изображение
            this.selectElement(imageContainer);

            this.updateToolbarStatus();
            this.showNotification('Изображение добавлено', 'success');
        }
    }
