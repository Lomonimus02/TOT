...
class SessionManager {
    ...
    // Обновление статуса зоны входа
    updateLoginZoneStatus() {
        // Ищем экземпляр FrameManager
        if (window.frameManager && typeof window.frameManager.updateLoginZoneStatus === 'function') {
            window.frameManager.updateLoginZoneStatus();
        } else {
            // Если FrameManager не найден, пробуем обновить напрямую
            setTimeout(() => {
                if (window.frameManager && typeof window.frameManager.updateLoginZoneStatus === 'function') {
                    window.frameManager.updateLoginZoneStatus();
                }
